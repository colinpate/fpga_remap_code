...
int main(int argc, char** argv)
{
	...
	for (int y = 0; y < fr_rows; y++) {
		for (int x = 0; x < fr_cols; x++) {
	        	...
	        	float offset = 400;
	        	float read_x = float(x) * (32.f) + offset;
	        	if (read_x >= 2048){
	        		read_x = 2048;
	        	}
	        	float read_y = float(y) * (32.f) + offset;
	        	if (read_y >= 1535){
	        		read_y = 1535;
	        	}
			float up_l_x = float(read_x) * 4;
			float up_l_y = float(read_y) * 4;
	
			float up_r_x = (32.f * 4.f ); //End of line relative to start (first row)
			float up_r_y = 0;
	
			float low_l_x = 0; //Start of line relative to start (last row)
			float low_l_y = (32.f * 4.f);
